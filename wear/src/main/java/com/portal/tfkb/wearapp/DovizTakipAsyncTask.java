...
public class DovizTakipAsyncTask extends AsyncTask<String, String, List<String>> {
	...
	private List<String> getDovizOranListFromInputStream(BufferedInputStream stream) {
		...
		try {
			...
			Element firstCube = (Element) document.getElementsByTagName("GetExchangeRateForCustomerXmlResponse").item(0);
			//Element secondCube = (Element) firstCube.getElementsByTagName("Currency").item(0);

			NodeList dovizOranNodeList = firstCube.getElementsByTagName("GetExchangeRateForCustomerXmlResult");
			...
			for (int i = 0; i < dovizOranNodeListLength ; i++) {
				...
				NodeList nodeListAlis=element.getElementsByTagName("a:BuyingRate");
				NodeList nodeListSatis=element.getElementsByTagName("a:SellingRate");
               ...
               double sell=Double.parseDouble(satis);
					...
					dovizOranList.add( "Euro ->"+"  Alış: "+String.format( "%.4f", buy )+"     Satış: "+String.format( " %.4f", sell ) );
